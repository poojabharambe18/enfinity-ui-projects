...
export const IMPS = ({ reqData }) => {
  ...
  const memoizedMetadata = useMemo(() => {
    const metadata = cloneDeep(IMPSGridMetaData);
    if (metadata?.gridConfig) {
      metadata.gridConfig.gridLabel = reqData?.TAB_DISPL_NAME ?? "IMPS";
    }
    return metadata;
  }, [data]);
  ...
  useEffect(() => {
    return () => {
      queryClient.removeQueries(["getIMPSList", authState?.user?.branchCode]);
    };
  }, []);
  return (
    <Fragment>
      ...
      <GridWrapper
        key={`IMPSGridMetaData ${data?.length}`}
