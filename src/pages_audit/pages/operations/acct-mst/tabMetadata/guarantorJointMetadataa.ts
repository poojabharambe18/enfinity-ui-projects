      ...
      _fields: [
          ...
          postValidationSetCrossFieldValues: async (
            ...
            if (Boolean(field?.value)) {
              if (
                field?.value ===
                dependentFieldsValues?.[
                  "JOINT_GUARANTOR_DTL.HIDDEN_CUSTOMER_ID"
                ]?.value
              ) {
                return {};
              }
        ...
        },
        {
          ...
          setValueOnDependentFieldsChange: (dependentFields) => {
            ...
            const CustomerId =
              dependentFields?.["JOINT_GUARANTOR_DTL.CUSTOMER_ID"]?.value;
        ...
        },
        {
          ...
          accountTypeMetadata: {
            ...
            postValidationSetCrossFieldValues: async (
              ...
              if (
                currentField?.value &&
                dependentFieldValues?.["JOINT_GUARANTOR_DTL.PATH_SIGN"]?.value
                  ?.length === 0
              ) {
                let buttonName = await formState?.MessageBox({
                  messageTitle: "Alert",
          ...
          accountCodeMetadata: {
            ...
            postValidationSetCrossFieldValues: async (
              ...
              if (
                currentField.value &&
                dependentFieldValues?.["JOINT_GUARANTOR_DTL.MEM_ACCT_TYPE"]
                  ?.value?.length === 0
              ) {
                let buttonName = await formState?.MessageBox({
                  messageTitle: "Alert",
                ...
                currentField?.value &&
                dependentFieldValues?.["JOINT_GUARANTOR_DTL.PATH_SIGN"]
                  ?.value &&
                dependentFieldValues?.["JOINT_GUARANTOR_DTL.MEM_ACCT_TYPE"]
                  ?.value
              ) {
                const reqParameters = {
                  ...
                  ACCT_CD:
                    utilFunction.getPadAccountNumber(
                      currentField?.value,
                      dependentFieldValues?.[
                        "JOINT_GUARANTOR_DTL.MEM_ACCT_TYPE"
                      ]?.optionData?.[0]
                    ) ?? "",
                  CUSTOMER_ID:
                    dependentFieldValues?.["JOINT_GUARANTOR_DTL.CUSTOMER_ID"]
                      ?.value ?? "",
        ...
        },
        {
          ...
          setFieldLabel: (dependentValue) => {
            const custType =
              dependentValue?.["JOINT_GUARANTOR_DTL.CUSTOMER_TYPE"]?.value;
            if (Boolean(custType)) {
              if (custType === "C") {
                return {
                  label: "InceptionDate",
        ...
        },
        {
          ...
          _optionsKey: "guarantorTabAreaDDW",
          // setValueOnDependentFieldsChange: (dependentFields) => {
        ...
        },
        {
          ...
          shouldExclude(fieldData, dependentFieldsValues, formState) {
            if (
              !Boolean(
                dependentFieldsValues?.["JOINT_GUARANTOR_DTL.FORM_60"]?.value
              ) ||
              dependentFieldsValues?.["JOINT_GUARANTOR_DTL.FORM_60"]?.value ===
                "N"
            ) {
