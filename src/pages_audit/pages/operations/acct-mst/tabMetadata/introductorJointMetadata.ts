      ...
      _fields: [
          ...
          postValidationSetCrossFieldValues: async (
            ...
            if (Boolean(field?.value)) {
              if (
                field?.value ===
                dependentFieldsValues?.[
                  "JOINT_INTRODUCTOR_DTL.HIDDEN_CUSTOMER_ID"
                ]?.value
              ) {
                return {};
              }
        ...
        },
        {
          ...
          setValueOnDependentFieldsChange: (dependentFields) => {
            ...
            const CustomerId =
              dependentFields?.["JOINT_INTRODUCTOR_DTL.CUSTOMER_ID"]?.value;
        ...
        },
        {
          ...
          accountTypeMetadata: {
            ...
            postValidationSetCrossFieldValues: async (
              ...
              if (
                currentField?.value &&
                dependentFieldValues?.["JOINT_INTRODUCTOR_DTL.PATH_SIGN"]?.value
                  ?.length === 0
              ) {
                let buttonName = await formState?.MessageBox({
                  messageTitle: "Alert",
          ...
          accountCodeMetadata: {
            ...
            postValidationSetCrossFieldValues: async (
              ...
              if (
                currentField.value &&
                dependentFieldValues?.["JOINT_INTRODUCTOR_DTL.MEM_ACCT_TYPE"]
                  ?.value?.length === 0
              ) {
                let buttonName = await formState?.MessageBox({
                  messageTitle: "Alert",
                ...
                currentField?.value &&
                dependentFieldValues?.["JOINT_INTRODUCTOR_DTL.PATH_SIGN"]
                  ?.value &&
                dependentFieldValues?.["JOINT_INTRODUCTOR_DTL.MEM_ACCT_TYPE"]
                  ?.value
              ) {
                const reqParameters = {
                  ...
                  ACCT_TYPE:
                    dependentFieldValues?.[
                      "JOINT_INTRODUCTOR_DTL.MEM_ACCT_TYPE"
                    ]?.value ?? "",
                  ACCT_CD:
                  ...
                  CUSTOMER_ID:
                    dependentFieldValues?.["JOINT_INTRODUCTOR_DTL.CUSTOMER_ID"]
                      ?.value ?? "",
        ...
        },
        {
          ...
          setFieldLabel: (dependentValue) => {
            const custType =
              dependentValue?.["JOINT_INTRODUCTOR_DTL.CUSTOMER_TYPE"]?.value;
            if (Boolean(custType)) {
              if (custType === "C") {
                return {
                  label: "InceptionDate",
        ...
        },
        {
          ...
          txtTransform: "uppercase",
          preventSpecialChars: sessionStorage.getItem("specialChar") || "",
          GridProps: { xs: 12, sm: 5, md: 3.2, lg: 3.2, xl: 3.5 },
        },
        {
          render: {
        ...
        },
        {
          ...
          _optionsKey: "instroductorAreaDDW",
          // setValueOnDependentFieldsChange: (dependentFields) => {
        ...
        },
        {
          ...
          shouldExclude(fieldData, dependentFieldsValues, formState) {
            if (
              !Boolean(
                dependentFieldsValues?.["JOINT_INTRODUCTOR_DTL.FORM_60"]?.value
              ) ||
              dependentFieldsValues?.["JOINT_INTRODUCTOR_DTL.FORM_60"]
                ?.value === "N"
            ) {
              return false;
            } else {
