      ...
      _fields: [
        ...
        },
        {
          ...
          postValidationSetCrossFieldValues: async (
            ...
            if (Boolean(field?.value)) {
              if (
                field?.value ===
                dependentFieldsValues?.["JOINT_GUARDIAN_DTL.HIDDEN_CUSTOMER_ID"]
                  ?.value
              ) {
                return {};
              }
        ...
        },
        {
          ...
          setValueOnDependentFieldsChange: (dependentFields) => {
            ...
            const CustomerId =
              dependentFields?.["JOINT_GUARDIAN_DTL.CUSTOMER_ID"]?.value;
        ...
        },
        {
          ...
          accountTypeMetadata: {
            ...
            postValidationSetCrossFieldValues: async (
              ...
              if (
                currentField?.value &&
                dependentFieldValues?.["JOINT_GUARDIAN_DTL.PATH_SIGN"]?.value
                  ?.length === 0
              ) {
                let buttonName = await formState?.MessageBox({
                  messageTitle: "Alert",
          ...
          accountCodeMetadata: {
            ...
            postValidationSetCrossFieldValues: async (
              ...
              if (
                currentField.value &&
                dependentFieldValues?.["JOINT_GUARDIAN_DTL.MEM_ACCT_TYPE"]
                  ?.value?.length === 0
              ) {
                let buttonName = await formState?.MessageBox({
                  messageTitle: "Alert",
                ...
                const reqParameters = {
                  COMP_CD: authState?.companyID ?? "",
                  BRANCH_CD:
                    dependentFieldValues?.["JOINT_GUARDIAN_DTL.PATH_SIGN"]
                      ?.value ?? "",
                  ACCT_TYPE:
                    dependentFieldValues?.["JOINT_GUARDIAN_DTL.MEM_ACCT_TYPE"]
                      ?.value ?? "",
                  ...
                  CUSTOMER_ID:
                    dependentFieldValues?.["JOINT_GUARDIAN_DTL.CUSTOMER_ID"]
                      ?.value ?? "",
                    ...
                    ? {
                        value:
                          utilFunction.getPadAccountNumber(
                            currentField?.value,
                            dependentFieldValues?.[
                              "JOINT_GUARDIAN_DTL.MEM_ACCT_TYPE"
                            ]?.optionData?.[0]
                          ) ?? "",
                        isFieldFocused: false,
                        ignoreUpdate: true,
        ...
        },
        {
          ...
          setFieldLabel: (dependentValue) => {
            const custType =
              dependentValue?.["JOINT_GUARDIAN_DTL.CUSTOMER_TYPE"]?.value;
            if (Boolean(custType)) {
              if (custType === "C") {
                return {
                  label: "InceptionDate",
        ...
        },
        {
          ...
          _optionsKey: "guardianAreaDDW",
          // setValueOnDependentFieldsChange: (dependentFields) => {
          // const pincode = dependentFields[""JOINT_GUARDIAN_DTL.PIN_CODE"]?.value;
          //   if (Boolean(pincode)) {
          //     if (pincode.length < 6) {
          //       return "";
          //     }
        ...
        },
        {
          ...
          shouldExclude(fieldData, dependentFieldsValues, formState) {
            if (
              !Boolean(
                dependentFieldsValues?.["JOINT_GUARDIAN_DTL.FORM_60"]?.value
              ) ||
              dependentFieldsValues?.["JOINT_GUARDIAN_DTL.FORM_60"]?.value ===
                "N"
            ) {
