...
const GuarantorJointTab = () => {
  ...
  useEffect(() => {
    handleCustFieldsReadOnlyctx(guarantorjoint_tab_metadata);
  }, []);
  const handleSave = (e) => {
    handleCurrFormctx({
      isLoading: true,
  ...
  const initialVal = useMemo(() => {
    ...
    if (
      ...
      !Boolean(AcctMSTState?.formDatactx["JOINT_GUARANTOR_DTL"])
    ) {
        ...
        onFormButtonClickHandel={async (id, dependentFields) => {
          if (id.slice(id.indexOf(".") + 1) === "CRDT_WORTHINESS_ignoreField") {
            ...
            setAccountNumberDetails({
              A_COMP_CD:
                dependentFields?.["JOINT_GUARANTOR_DTL.PATH_PHOTO"]?.value ??
                "",
              A_BRANCH_CD:
                dependentFields?.["JOINT_GUARANTOR_DTL.PATH_SIGN"]?.value ?? "",
              A_ACCT_TYPE:
                dependentFields?.["JOINT_GUARANTOR_DTL.MEM_ACCT_TYPE"]?.value ??
                "",
              A_ACCT_CD:
                dependentFields?.["JOINT_GUARANTOR_DTL.MEM_ACCT_CD"]?.value ??
                "",
              TAB: "GUARANTORTAB",
            });
          }
