...
const GuardianJointTab = () => {
  ...
  useEffect(() => {
    handleCustFieldsReadOnlyctx(guardianjoint_tab_metadata);
  }, []);
  const handleSave = (e) => {
    handleCurrFormctx({
      isLoading: true,
  ...
  const initialVal = useMemo(() => {
    ...
    if (
      ...
      !Boolean(AcctMSTState?.formDatactx["JOINT_GUARDIAN_DTL"])
    ) {
    ...
    <Grid>
      <FormWrapper
        ...
        formState={{
          ...
          acctDtlReqPara: {
            MEM_ACCT_CD: {
              ACCT_TYPE: "JOINT_GUARDIAN_DTL.MEM_ACCT_TYPE",
              BRANCH_CD: "JOINT_GUARDIAN_DTL.PATH_SIGN",
              SCREEN_REF: docCD ?? "",
            },
          },
        }}
        ...
        onFormButtonClickHandel={async (id, dependentFields) => {
          if (id.slice(id.indexOf(".") + 1) === "CRDT_WORTHINESS_ignoreField") {
            ...
            setAccountNumberDetails({
              A_COMP_CD:
                dependentFields?.["JOINT_GUARDIAN_DTL.PATH_PHOTO"]?.value ?? "",
              A_BRANCH_CD:
                dependentFields?.["JOINT_GUARDIAN_DTL.PATH_SIGN"]?.value ?? "",
              A_ACCT_TYPE:
                dependentFields?.["JOINT_GUARDIAN_DTL.MEM_ACCT_TYPE"]?.value ??
                "",
              A_ACCT_CD:
                dependentFields?.["JOINT_GUARDIAN_DTL.MEM_ACCT_CD"]?.value ??
                "",
              TAB: "GUARDIANTAB",
            });
          }
