...
const IntroductorJointTab = () => {
  ...
  useEffect(() => {
    handleCustFieldsReadOnlyctx(introductorjoint_tab_metadata);
  }, []);
  const handleSave = (e) => {
    handleCurrFormctx({
      isLoading: true,
  ...
  const initialVal = useMemo(() => {
    ...
    return AcctMSTState?.isFreshEntryctx
      ...
      : {
          JOINT_INTRODUCTOR_DTL: [...formData],
          HIDDEN_COMP_CD: authState?.companyID ?? "",
          HIDDEN_IsFreshEntry: AcctMSTState?.isFreshEntryctx ?? "",
        };
  }, [
        ...
        onFormButtonClickHandel={async (id, dependentFields) => {
          if (id.slice(id.indexOf(".") + 1) === "CRDT_WORTHINESS_ignoreField") {
            ...
            setAccountNumberDetails({
              A_COMP_CD:
                dependentFields?.["JOINT_INTRODUCTOR_DTL.PATH_PHOTO"]?.value ??
                "",
              A_BRANCH_CD:
              ...
              A_ACCT_TYPE:
                dependentFields?.["JOINT_INTRODUCTOR_DTL.MEM_ACCT_TYPE"]
                  ?.value ?? "",
              A_ACCT_CD:
                dependentFields?.["JOINT_INTRODUCTOR_DTL.MEM_ACCT_CD"]?.value ??
                "",
              TAB: "JOINTTAB",
            });
          }
